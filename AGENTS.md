...
- 下载 comigo.xyz 的平台发布包，只提取 comi。PATH 含 `/usr/bin` 时默认安装 `/usr/bin/comi`，否则使用 `~/.local/bin/comi`；pkexec 授权系统文件复制或 UFW 规则变更，已有目标不覆盖。
...
- `/api/server/update` 和 `comi desktop check-update` 复用版本比较；调用 desktop 前检测 CLI 能力。
