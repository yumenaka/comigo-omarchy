...
未连接时打开面板默认进入设置页；面板打开期间断线保留当前页面。设置页按钮在已连接时显示“断开”，未连接时显示“连接”；忙碌时仍保持可用样式并接受点击，点击连接会取消旧请求并立即重试。主动断开会停止插件请求，点击“连接”恢复；不会停止 Comigo 服务。插件设置由 Omarchy 宿主保存。设置页顶部显示连接状态、对应提示与服务地址，服务可达时可直接打开浏览器。 设置页不显示登录按钮或登录标题。打开插件或保存连接地址后，若服务要求认证，自动弹出账号密码模态框；取消后不会因后台轮询重复弹出，再次打开插件或保存地址可重新提示。已登录时保留退出登录。弹窗标题为“登录（如果已设置账户密码）”；成功后关闭，失败时在框内提示，支持取消、Esc 和点击外部关闭，提交或关闭后清空密码。

## 功能

- 四页：概览、状态、服务、设置。支持阅读链接、二维码、服务器 IP 切换、版本、书籍与连接数量、速度和累计流量。
- 仅当已保存的连接地址主机为 `127.0.0.1` 或 `localhost` 时在“服务”页顶部显示局域网共享开关，避免远程关闭后无法恢复；只读模式禁止修改服务设置。配置文件信息只读展示，详细配置进入 Comigo 网页。
