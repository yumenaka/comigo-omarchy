...
- **Comigo**：本机模式需要 **v1.3.5 或以上**的 `comi` 二进制文件，缺少时可在“服务”页安装；添加插件不会自动安装它。远程模式需要已运行且满足相同最低版本要求的服务端，本机无需安装二进制文件。
- **可选功能**：安装二进制文件使用 `tar` 和系统基础工具，安装到系统目录时还需 `pkexec` 授权；防火墙操作使用 `ufw`、`ip` 和 `pkexec`。

## 使用方法

### 本机模式（默认）

1. 未安装 Comigo 时，在“服务”页安装。已有安装会自动识别；识别失败时，在“设置”中指定二进制文件路径。
2. 安装完成后自动启动 Comigo。已经安装的用户点击“启动”。
...
- **服务管理**：本机模式可停止、重启和查看日志，两种模式均可检查更新。插件只能控制自己启动的进程，更新检查不会自动升级。
